          ...
          return (
            <div key={day} className={cn(
              ...
              <div className={cn(
                ...
                isToday ? "text-brand-600" : "text-slate-400 dark:text-slate-500"
              )}>
                {day}
              </div>
              ...
              <div className="space-y-3">
                {dayClasses.length === 0 ? (
                  <div className="p-4 rounded-xl border border-dashed border-slate-200 dark:border-slate-800 text-center text-xs text-slate-400 dark:text-slate-600">
                    No classes
                  </div>
                ) : (
                  dayClasses.map((session) => (
                    <div
                      ...
                      className="p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 border-l-4 rounded-xl shadow-sm hover:shadow-md transition-all group relative"
                    >
                      <div className="flex justify-between items-start gap-2">
                        <h5 className="font-semibold text-sm text-slate-800 dark:text-white truncate flex-1">{session.name}</h5>
                        {onDelete && (
                          <button 
